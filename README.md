...
in which you can decide whether or not to add a title on your plot. If `title = TRUE`, the function will plot "DVHs for plan *plan.name*" as title, if `title = FALSE` the function won't plot any title, if instead `title = chr` is set to any string, the function will plot the string you provided as title (default value is `title = FALSE`).
...
```         
filtered_plan <- selectDVHsStructures(plan = my_plan, keep.structures = c("CTV", "PTV"))
```
...
```         
my_robustness <- readRobustness(robustness.csv = "path/to/robustness.csv", renamed.structures = new.names)
```

and to plot robustness DVHs, use the `plotRobustness` function:

```         
plotRobustness(robustness = my_robustness, title = TRUE, robustness.name = "My Plan")
```
...
```         
plotRobustnessSpread(robustness = my_robustness, title = TRUE, robustness.name = "My Plan")
```
